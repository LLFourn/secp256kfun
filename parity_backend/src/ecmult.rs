...
use subtle::Choice;
use crate::group::{
    Affine, Jacobian, AffineStorage, globalz_set_table_gej,
};
...
mod alloc_table {
    use crate::{
        ...
        ecmult::{ECMULT_TABLE_SIZE_G, odd_multiples_table,ECMultContext, ECMultGenContext, GEN_BLIND, GEN_INITIAL}
    };
    use alloc::{
        alloc::{alloc, Layout},
        boxed::Box,
    ...
    fn set_all_gej_var(a: &[Jacobian]) -> Vec<Affine> {
    ...
    /// normalized).
    fn inv_all_var(fields: &[Field]) -> Vec<Field> {
        if fields.len() == 0 {
            return Vec::new();
        }
