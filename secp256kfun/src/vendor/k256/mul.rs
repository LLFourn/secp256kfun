...
pub use super::{ProjectivePoint, Scalar};
...
fn decompose_scalar(k: &Scalar) -> (Scalar, Scalar) {
    ...
    let c1 = Scalar::mul_shift_var(k, &G1, 384) * MINUS_B1;
    let c2 = Scalar::mul_shift_var(k, &G2, 384) * MINUS_B2;
    let r2 = c1 + c2;
...
#[allow(unused)]
pub fn lincomb_generic<const N: usize>(
    xs: &[&ProjectivePoint; N],
    ks: &[&Scalar; N],
) -> ProjectivePoint {
    let rs = static_map(
