...
impl Point<Normal, Public, NonZero> {
    ...
    /// # use sha2::Sha256;
    /// let point = Point::hash_to_curve_rfc9380::<Sha256>(b"hello world", b"myapp-v1");
    /// ```
    ///
    /// [`hash_to_curve`]: Self::hash_to_curve
    /// [`hash_to_curve_rfc9381_tai`]: Self::hash_to_curve_rfc9381_tai
    pub fn hash_to_curve_rfc9380<H>(msg: &[u8], dst: &[u8]) -> Point<NonNormal, Public, NonZero>
    where
    ...
    /// runtime based on input (see [`hash_to_curve_rfc9380`] for details), this is rarely
    /// a practical concern.
    ///
    ...
    /// ```
    ///
    /// [`hash_to_curve_rfc9380`]: Self::hash_to_curve_rfc9380
    ...
    /// algorithm as used in SECP256K1_SHA256_TAI.
    ///
    ...
    /// See [`hash_to_curve_rfc9380`] for discussion of DoS considerations.
    ///
    ...
    pub fn hash_to_curve_rfc9381_tai<H: Hash32>(
        ...
        for counter in 0u8..u8::MAX {
            ...
            // Always use 0x02 (even y) as per RFC 9381 try-and-increment
            let mut bytes = [0u8; 33];
            bytes[0] = 0x02;
            bytes[1..].copy_from_slice(&hash_bytes);
