...
#[test]
fn test_hash_to_curve_rfc9380_test_vectors() {
    ...
    for (i, (msg, expected_x, expected_y)) in test_vectors.iter().enumerate() {
        let point = Point::hash_to_curve_rfc9380::<Sha256>(msg, dst).normalize();
...
proptest! {
    #[test]
    fn test_hash_to_curve_rfc9380_properties(
        ...
        let point1 = Point::hash_to_curve_rfc9380::<Sha256>(&msg1, &dst1);
        let point1_again = Point::hash_to_curve_rfc9380::<Sha256>(&msg1, &dst1);
        assert_eq!(point1, point1_again, "hash_to_curve_rfc9380 should be deterministic");

        // Point should be NonNormal
        let _: Point<NonNormal, Public, NonZero> = point1;

        // Points should never be zero
        assert!(!point1.is_zero(), "hash_to_curve_rfc9380 should never produce zero point");

        // Different messages with same DST should produce different points (with high probability)
        if msg1 != msg2 {
            let point2 = Point::hash_to_curve_rfc9380::<Sha256>(&msg2, &dst1);
            assert_ne!(point1, point2, "Different messages should produce different points");
            assert!(!point2.is_zero(), "hash_to_curve_rfc9380 should never produce zero point");
        }

        // Same message with different DSTs should produce different points (with high probability)
        if dst1 != dst2 {
            let point3 = Point::hash_to_curve_rfc9380::<Sha256>(&msg1, &dst2);
            assert_ne!(point1, point3, "Different DSTs should produce different points");
            assert!(!point3.is_zero(), "hash_to_curve_rfc9380 should never produce zero point");
        }
    }
