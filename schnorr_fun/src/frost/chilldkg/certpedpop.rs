...
pub struct CertifiedKeygen<S: CertificationScheme> {
    pub(crate) input: AggKeygenInput,
    pub(crate) certificate: Certificate<S::Signature>,
    /// The outputs from successful verification, indexed by certifying party's public key
    pub(crate) outputs: BTreeMap<Point, S::Output>,
}
...
pub fn simulate_keygen<H: Hash32, NG: NonceGen, S: CertificationScheme>(
    ...
    for (party_index, enckey) in &receiver_enckeys {
        let (share_receiver, cert) =
            ShareReceiver::receive_share(schnorr, cert_scheme, *party_index, enckey, &agg_input)
                .unwrap();
...
mod test {
    ...
    proptest! {
        ...
        fn run_simulate_keygen(
            ...
            let (certified_keygen, paired_secret_shares_and_keys) = simulate_keygen(&schnorr, &schnorr, threshold, n_receivers, n_generators, &mut rng);
    ...
    fn vrf_certified_keygen_randomness_beacon() {
        ...
        let (certified_keygen, _) = simulate_keygen(
            &schnorr,
            &vrf_certifier,
            threshold,
            n_receivers,
