...
pub struct Contributor {
    pub(crate) inner: simplepedpop::Contributor,
}
...
pub struct AggKeygenInput {
    pub(crate) inner: simplepedpop::AggKeygenInput,
    pub(crate) encrypted_shares: BTreeMap<ShareIndex, (Point, Scalar<Public, Zero>)>,
    pub(crate) encryption_nonces: Vec<Point>,
}
    ...
    H: Hash32,
{
    ...
    let paired_secret_share = simplepedpop::receive_share(schnorr, &agg_input.inner, secret_share)?;
    ...
    NG: NonceGen,
{
    ...
    for (party_index, enckey) in receiver_enckeys {
        let paired_secret_share = receive_share(schnorr, party_index, &enckey, &agg_input).unwrap();
